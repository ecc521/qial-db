...
var userDetails;
var accountPopup = document.createElement("iframe");
...
function syncUserDetails() {
  return _syncUserDetails.apply(this, arguments);
}

function _syncUserDetails() {
  _syncUserDetails = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
    var req, resp;
...
var menuDiv = document.getElementById("deleteMenuDiv");
var info = document.getElementById("deleteInfo");
var results = document.getElementById("deleteResuts");
var deleteButton = document.getElementById("deleteButton");
menuDiv.remove();
var menu = new window.Overlay();
var toggleButton = document.getElementById("toggleDelete");
...
function getDownloadData() {
  var items = itemHolder.filter(function (item) {
  ...
  info.innerHTML = "You have selected ".concat(items.length, " files totalling ").concat(numberPrettyBytesSI(totalSize), ". ");
  return items;
}

toggleButton.addEventListener("click", getDownloadData);
window.addEventListener("bulkSelectionUsed", getDownloadData);
deleteButton.addEventListener("click", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
  var items, names, i, request, response;
  return regeneratorRuntime.wrap(function _callee$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          items = getDownloadData();
          ...
          return _context.abrupt("return", alert("Please exit the download menu and select some items. "));
        ...
        case 3:
          ...
          if (!(confirm("Are you sure you want to delete " + items.length + " files?") && confirm("Are you ABSOLUTELY SURE you want to delete " + items.length + " files? (This can't be undone)"))) {
            _context.next = 19;
            break;
          }
...
var getDownloadTemplate = __webpack_require__(6302);

var downloadMenuDiv = document.createElement("div");
document.body.appendChild(downloadMenuDiv);
var downloadInfo = document.createElement("p");
downloadMenuDiv.appendChild(downloadInfo);
var downloadZip = document.createElement("button");
downloadZip.innerHTML = "Download ZIP";
downloadMenuDiv.appendChild(downloadZip);
var downloadNodejsScript = document.createElement("button");
...
var downloadMenu = new window.Overlay();
...
function getSelectionJSONMetadata() {
  var itemsSelected = parentHolder.filter(function (item) {
...
function getSelectionCSVMetadata() {
  ...
  var csvLines = window.csvSources["Mice"].split("\n");
  var output = csvLines[0];
  itemsSelected.forEach(function (item) {
    var _item$csvSources;

    if (item !== null && item !== void 0 && (_item$csvSources = item.csvSources) !== null && _item$csvSources !== void 0 && _item$csvSources["Mice"].lineNumber) {
      var _item$csvSources2;

      output += "\n" + csvLines[item === null || item === void 0 ? void 0 : (_item$csvSources2 = item.csvSources) === null || _item$csvSources2 === void 0 ? void 0 : _item$csvSources2["Mice"].lineNumber];
    }
  });
  return output;
}
...
function offerAdditionalFiles() {
  downloadFullJSON.innerHTML = "Download JSON Metadata File (".concat(numberPrettyBytesSI(getFullJSONMetadata().length), ")");
  downloadSelectedJSON.innerHTML = "Download JSON Metadata for Selection (".concat(numberPrettyBytesSI(getSelectionJSONMetadata().length), ")");
  downloadFullCSV.innerHTML = "Download CSV Metadata File (".concat(numberPrettyBytesSI(getFullCSVMetadata().length), ")");
  downloadSelectedCSV.innerHTML = "Download CSV Metadata for Selection (".concat(numberPrettyBytesSI(getSelectionCSVMetadata().length), ")");
}
...
function getDownloadData() {
  ...
  downloadInfo.innerHTML = "You have selected ".concat(items.length, " files totalling ").concat(numberPrettyBytesSI(totalSize), ". ");
  return items;
}
...
downloadZip.addEventListener("click", function () {
  var items = getDownloadData();

  if (items.length === 0) {
    return alert("Please exit the download menu and select some items to download. ");
  }
...
downloadNodejsScript.addEventListener("click", function () {
  ...
  var str = getDownloadTemplate(urls);
...
function generateNeuroglancerLink(_ref) {
  ...
  var obj = {
    ...
    "position": [100.5, 100.5, 5.5],
    "layers": [{
      "type": "image",
      "source": {
        "url": "precomputed://".concat(window.location.href, "cache/precomputed/").concat(fileName),
        "transform": {
  ...
  if (labelName) {
    obj.layers.push({
      "type": "segmentation",
      "source": {
        "url": "precomputed://".concat(window.location.href, "cache/precomputed/").concat(labelName),
        "subsources": {
          "default": true,
          "bounds": true
        },
  ...
  var appspotDemoUrl = "https://neuroglancer-demo.appspot.com/"; //Appspot demo is hosted by Google.
  ...
  var selfHostedUrl = window.location.href + "neuroglancer/dist/min/";
  var neuroglancerLink = appspotDemoUrl + "#!" + encodeURI(JSON.stringify(obj));
...
var numericAxes = [];
var nonNumericAxes = [];
var animals = window.data.filter(function (a) {
  return a.type === "animal";
});
animals.forEach(function (animal) {
  for (var prop in animal) {
    var value = animal[prop]; //All numbers or strings that convert to number.
...
var axisSelectorDiv = document.createElement("div");
graphsDiv.appendChild(axisSelectorDiv);
var plotlyContainer = document.createElement("div");
graphsDiv.appendChild(plotlyContainer); //Generates the selector for an axis.
  ...
  if (multiple) {
    ...
    props.forEach(function (prop) {
      ...
      box.addEventListener("change", function () {
        ...
        for (var _prop in propsObj) {
          var _box = propsObj[_prop];

          if (_box.checked) {
            selectedProps.push(_prop);
          }
        }
...
function genGraph() {
  var items = window.lastSearchItems;
  ...
  if (graphType === "Violin") {
    ...
    items.forEach(function (item) {
      var val = item[axes[2]]; //If there isn't a z axis, this still works, the only z is "undefined" (and with only one z, labels aren't displayed)

      if (axes[2] !== undefined && val === undefined) {
        return;
      }
    ...
    for (var _prop2 in groups) {
      var groupItems = groups[_prop2];
      groups[_prop2] = {
        data: groupItems.map(function (item) {
          var xVal = item[axes[0]];
          var yVal = item[axes[1]];
          return {
            x: [xVal],
            y: [yVal]
          };
    ...
    var props = Object.keys(groups);
    var useSplitViolin = props.length === 2;
    layout = {
      title: "Violin Plot",
    ...
    props.forEach(function (prop, index) {
      ...
      var info = {
        ...
        points: "all",
        pointpos: -.5,
        //TODO: Compute something that works here - off to the side, but minimally.
      ...
      if (useSplitViolin && index === 1) {
        if (info.side === "positive") {
          info.side = "negative";
        } else if (info.side === "negative") {
          info.side = "positive";
        }
    ...
    (function () {
      ...
      var _loop = function _loop(groupName) {
        ...
        axes[1].forEach(function (yProp) {
          ...
          traceItems.forEach(function (data) {
            data.x.forEach(function (xVal, i) {
              var correspondingY = data.y[i];
  ...
  // }
      ...
      throw "Unsupported graphType " + graphType;
    }
...
function setGraphOptions() {
  while (axisSelectorDiv.lastChild) {
    axisSelectorDiv.lastChild.remove();
  }

  var graphType = graphTypeSelector.value;
  var graphInfo = graphOptions[graphType];
  console.log(graphInfo);
  axes = ["x", "y", "z"].filter(function (axis) {
    return graphInfo[axis];
  }) //Filter out any axes that aren't used
  .map(function (axis, index) {
    var info = graphInfo[axis];
    var params = [];
    ...
    var elem = axisSelector.apply(void 0, params);
...
var overlayClassInstances = []; //Potential memory leak, so we need to make sure Overlays are NEVER dynamically created.
...
module.exports = /*#__PURE__*/function () {
  ...
  _createClass(Overlay, [{
    key: "show",
    value: function show(content, hideCallback) {
      overlayClassInstances.forEach(function (overlayClassInstance) {
...
var generateNeuroglancerLink = __webpack_require__(5754);
...
function Item(item) {
  ...
  var addText = function addText(text) {
    var container = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.infoCard;
  ...
  this.componentRows = [];
  item["Image Count"] = String((_item$views$length = item === null || item === void 0 ? void 0 : (_item$views = item.views) === null || _item$views === void 0 ? void 0 : _item$views.length) !== null && _item$views$length !== void 0 ? _item$views$length : "");
  ...
  if (item.type === "file") {
    ...
    addText("File Name: ".concat(item.name));
    addText("Size: ".concat(window.numberPrettyBytesSI(item.size, 2)));
    addText("Last Modified: ".concat(new Date(item.lastModified).toDateString()));
    ...
    var _item$csvSources, _item$csvSources$Mice;
    ...
    var getFileSize = function getFileSize(fileName) {
      ...
      try {
        return item.componentFiles.find(function (componentFile) {
          return componentFile.name === fileName;
        }).size;
      } catch (e) {
    ...
    var addThumbnails = function addThumbnails(view, container, fileSize) {
      if (!view.thumbnails) {
        console.warn("No Thumbnails");
        return;
      }

      view.thumbnails.forEach(function (fileName) {
        var img = document.createElement("img");
        img.src = "cache/thumbnails/".concat(fileName);
    ...
    var createButton = function createButton(view, fileSize) {
      var preview = document.createElement("button");
      preview.className = "neuroglancerLink"; //Neuroglancer doesn't support 64 bit floats - so we can't always provide a precomputed that is
      ...
      if (!view.neuroglancer) {
        preview.innerHTML = "Download ".concat(view.name);
        ...
        preview.innerHTML = "View ".concat(view.name, " in Neuroglancer");
        preview.addEventListener("click", function () {
          window.open(generateNeuroglancerLink({
    ...
    this.componentFiles = this.componentFiles.map(function (component) {
      component.parent = _this;
      var item = new Item(component);

      _this.componentRows.push(item.row);

      return item;
    });

    var setComponentVisibility = function () {
      if (this.checkbox.checked === false) {
        this.componentFiles.forEach(function (file) {
    ...
    this.checkbox.addEventListener("change", setComponentVisibility); //Add link to edit the Animal's information.

    var animalName = addText("Animal: ".concat(item.Animal));
    var editLink = document.createElement("a");
    var editLinkSrc = (_item$csvSources = item.csvSources) === null || _item$csvSources === void 0 ? void 0 : (_item$csvSources$Mice = _item$csvSources["Mice"]) === null || _item$csvSources$Mice === void 0 ? void 0 : _item$csvSources$Mice.editUrl;
    ...
    addText("Sex: ".concat(item.Sex));
    addText("Genotype: ".concat(item.Genotype));
    addText("Weight: ".concat(item.weight));
    addText("DOB: ".concat(item.DOB));
    addText("File Count: ".concat(item.componentFiles.length));

    if (item.Sex === "male") {
      this.row.style.backgroundColor = "#eeeeff";
    } else if (item.Sex === "female") {
      this.row.style.backgroundColor = "#ffeeee";
    }
...
window.drawCards = function drawCards(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    item.componentRows.forEach(function (row) {
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 5:
          ...
          search = __webpack_require__(9821);
...
var _require = __webpack_require__(4935),
    genGraph = _require.genGraph,
    graphsDiv = _require.graphsDiv;
...
var searchOptions = document.createElement("div");
search.appendChild(searchOptions);
var expanded;

function setExpanded() {
  var expand = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : expanded;
  expanded = expand;

  if (!expand) {
    searchOptions.style.display = graphsDiv.style.display = "none";
    searchDropdown.innerHTML = "<span>Search Filters/Graphs (Click to Expand)</span><span>".concat(infoText, "</span><span>\u2B07\uFE0E</span>");
  } else {
    searchOptions.style.display = graphsDiv.style.display = "";
    searchDropdown.innerHTML = "<span>Search Filters/Graphs (Click to Collapse)</span><span>".concat(infoText, "</span><span>\u2B06\uFE0E</span>");
  }
}
...
var searchFilters = []; //Functions to filter list through
...
function generateSearchOptions(items) {
  var options = [{
    ...
    optionName: "DOB",
    type: "range",
    convertFrom: function convertFrom(a) {
  ...
  options.forEach(function (_ref) {
    var optionName = _ref.optionName,
        ...
        displayName = _ref$displayName === void 0 ? optionName : _ref$displayName;
    var itemBar = document.createElement("div");
    itemBar.className = "searchItemBar";
    var itemBarInfo = document.createElement("span");
    itemBarInfo.innerHTML = "".concat(displayName, ": ");
    itemBar.appendChild(itemBarInfo);
    searchOptions.appendChild(itemBar); //TODO: Disable all other checkboxes when this is on in the searchFilter.
    //We'll show the difference that pushing a single checkbox would make as a +/- X (ex, +9 or -2).

    var enabledCheckbox = createCheckbox();
    enabledCheckbox.addEventListener("change", processSearch);
    itemBar.appendChild(enabledCheckbox);
    var enabledLabel = createLabel(enabledCheckbox);
    itemBar.appendChild(enabledLabel); //Extract all the different properties and the amount of times that they occur.

    function getPossibilities(items) {
      var possibilities = {};
      items.forEach(function (item) {
        if (item[optionName] === undefined || item[optionName] === "") {
          return;
        }
    ...
    function obtainOptions() {
      ...
      for (var _optionName in optionElems) {
        obj[_optionName] = optionElems[_optionName].checkbox.checked;
      }
    ...
    if (type === "of") {
      searchFilter = function searchFilter(items) {
        var searchEnabled = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : enabledCheckbox.checked;
        var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : obtainOptions();
        ...
        return items.filter(function (item) {
          return options[item === null || item === void 0 ? void 0 : item[optionName]];
        });
      };
    } else if (type === "range") {
      searchFilter = function searchFilter(items) {
        var searchEnabled = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : enabledCheckbox.checked;
        var minSelection = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : selects[0].value;
        var maxSelection = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : selects[1].value;
        ...
        if (convertFrom) {
          if (!minSelection) {
            minSelection = -Infinity;
          } else {
            minSelection = convertFrom(minSelection);
          }

          if (!maxSelection) {
            maxSelection = Infinity;
          } else {
            maxSelection = convertFrom(maxSelection);
          }
        }

        if (maxSelection < minSelection) {
          var temp = maxSelection;
    ...
    function setPossibilities() {
      var currentItems = items; //Use all filters but this one in order to show how many match the selection options given the selectors already picked. This will then be passed to getPossibilities.

      searchFilters.filter(function (filter) {
      ...
      var possibilities = getPossibilities(items);
      var possibilitiesArray = Object.keys(possibilities);

      if (convertFrom) {
        possibilitiesArray = possibilitiesArray.filter(function (poss) {
          return poss;
        }); //Filter blanks from range search. TODO: Suppress all in getPossibilities

        possibilitiesArray = possibilitiesArray.sort(function (a, b) {
          return convertFrom(a) - convertFrom(b);
        });
      }

      var currentPossibilities = getPossibilities(currentItems);
      possibilitiesArray.forEach(function (value) {
        if (type === "of") {
          var elems = optionElems[value];
          ...
          elems.label.innerHTML = "".concat(value, " (").concat(currentPossibilities[value] || 0, ")");
        } else if (type === "range") {
          ;
          [0, 1].forEach(function (iterationNum) {
            var label = document.getElementsByName("label" + optionName)[iterationNum];

            if (!label) {
              var _label = createLabel();

              _label.innerHTML = iterationNum ? "And: " : "Between: ";

              _label.setAttribute("name", "label" + optionName);

              itemBar.appendChild(_label);
              itemBar.appendChild(selects[iterationNum]);
            }

            var option = document.getElementsByName(value + optionName)[iterationNum];
            ...
            var amount = searchFilter(currentItems, true, iterationNum === 0 ? value : undefined, iterationNum === 1 ? value : undefined).length;
            option.innerHTML = "".concat(value, " (").concat(amount, ")");
          });
        }

        enabledLabel.innerHTML = "Enable Filter (".concat(searchFilter(currentItems, true).length - currentItems.length, ")");
      });
    }
...
function processSearch() {
  ...
  drawCards(items);
  infoText = "Displaying ".concat(items.length, " of ").concat(window.data.length, " Items");
  setExpanded();
  window.dispatchEvent(new Event("searchProcessed"));
}

module.exports = {
  generateSearchOptions: generateSearchOptions,
...
function setAll() {
  ...
  parentHolder.forEach(function (item) {
...
var uploadMenuDiv = document.getElementById("uploadMenu");
...
var uploadMenu = new window.Overlay();
var toggleUpload = document.getElementById("toggleUpload");
...
window.numberPrettyBytesSI = function numberPrettyBytesSI() {
...
function _uploadFile(_x, _x2, _x3, _x4, _x5) {
  return _uploadFile2.apply(this, arguments);
}

function _uploadFile2() {
  _uploadFile2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(file, start, end, setFileProgress, last) {
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return new Promise(function (resolve, reject) {
              ...
              request.upload.addEventListener('progress', function (e) {
                var total_loaded = start + e.loaded;
                var total_percent = total_loaded / file.size * 100;
...
function uploadFile(_x6, _x7) {
  return _uploadFile3.apply(this, arguments);
}

function _uploadFile3() {
  _uploadFile3 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(file, _ref) {
    var filenumber, setProgress, firstChunkSize, chunkSize, currentPos, totalChunks, progress, label, br, setFileProgress, currentEnd, complete, res, p;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            setFileProgress = function _setFileProgress(percentage, bytesLoaded) {
              setProgress(filenumber, percentage);
              progress.value = percentage;
              label.innerHTML = "Uploading ".concat(file.name, " - ").concat(Math.round(percentage * 10) / 10, "% (").concat(numberPrettyBytesSI(bytesLoaded, 2), " of ").concat(numberPrettyBytesSI(file.size, 2), ")");
            };

            filenumber = _ref.filenumber, setProgress = _ref.setProgress;
            //Split into chunks
            firstChunkSize = 2e5;
            chunkSize = 5e6;
            currentPos = 0;
          ...
          case 12:
            ...
            currentEnd = currentPos + (currentPos === 0 ? firstChunkSize : chunkSize);
            ...
            _context2.next = 19;
            return _uploadFile(file, currentPos, currentEnd, setFileProgress, complete);
          ...
          case 19:
            ...
            if (res instanceof Error) {
              p.innerHTML = "Error: ".concat(res.message);
            } else {
              p.innerHTML = "Error (Status ".concat(res.status, "): ").concat(res.message); //TODO: Retry timeouts and disconnects.
            }
...
function uploadFiles(_x8) {
  return _uploadFiles.apply(this, arguments);
}

function _uploadFiles() {
  _uploadFiles = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee4(files) {
    var progress, label, setProgress, i, maxConcurrent, originalConcurrent, startNext, _startNext, _i;

    return regeneratorRuntime.wrap(function _callee4$(_context4) {
      while (1) {
        switch (_context4.prev = _context4.next) {
          case 0:
            _startNext = function _startNext3() {
              _startNext = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3() {
                var filenumber, file, elem, response;
                ...
                }, _callee3, null, [[7, 14, 17, 22]]);
              }));
              return _startNext.apply(this, arguments);
            };

            startNext = function _startNext2() {
              return _startNext.apply(this, arguments);
            };

            setProgress = function _setProgress(filenumber) {
              var percentage = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
              label.innerHTML = "Uploading File ".concat(filenumber + 1, " of ").concat(files.length, "...");
              progress.value = (filenumber + percentage / 100) / files.length * 100;
            };

            //Clear the current progress history.
            uploadResults.innerHTML = ""; //Begin uploading... We'll do this one file at once for now.

            progress = createProgressBar();
            uploadResults.appendChild(progress);
            label = document.createElement("label");
            uploadResults.appendChild(label);
            uploadResults.appendChild(document.createElement("br")); //This isn't ideal with paralell uploads, but it works OK.

            i = 0;
            maxConcurrent = 3;
            originalConcurrent = maxConcurrent;

            //let scope bounds to loop, so the higher scope i is safe.
            for (_i = 0; _i < originalConcurrent; _i++) {
              startNext();
            }
...
uploadButton.addEventListener("click", function () {
  if (currentFilesReady.length === 0) {
    return alert("You need to select some files to upload. ");
  }
...
window.Overlay = __webpack_require__(7013);
